...
class WebSocketHandler:
    ...
    def detect_and_translate(self, text, detected_lang):
        ...
        try:
            ...
            if lang not in ['ko', 'korean', 'en', 'english']:
                ...
                if korean_chars > len(text) * 0.3:
                    ...
                    lang = 'en'
                logger.info(f"Unknown language '{detected_lang}', auto-detected as: {lang}")
    ...
    async def send_result(self, iteration_output):
        ...
        if iteration_output:
            ...
            # Send partial_cumulative or final based on whether it's committed
            if hasattr(iteration_output, 'committed') and iteration_output['committed']:
                ...
                # Partial result (번역 없이)
                await self.send_message({
                    'type': 'partial_cumulative',
                    'start': start_ms,
                    'end': end_ms,
                    'original': text,
                    'polished': text
