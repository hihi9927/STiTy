...
function loadSettings() {
  ...
  if (savedHost) {
    DOM.serverHostInput.value = savedHost;
  }

  const savedPort = localStorage.getItem('serverPort');
  if (savedPort) {
    DOM.serverPortInput.value = savedPort;
  }
...
function setupEventListeners() {
  ...
  // 서버 호스트 변경
  DOM.serverHostInput.addEventListener('change', (e) => {
    const value = e.target.value.trim();
    localStorage.setItem('serverHost', value);
    ipcRenderer.send('server-host-changed', value);
  });

  // 서버 포트 변경
  DOM.serverPortInput.addEventListener('change', (e) => {
    const value = e.target.value.trim();
    localStorage.setItem('serverPort', value);
    ipcRenderer.send('server-port-changed', value);
  });
