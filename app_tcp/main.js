...
function setupIpcListeners() {
  ...
  ipcMain.on('server-host-changed', (event, value) => {
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('server-host-changed', value);
    }
  });

  ipcMain.on('server-port-changed', (event, value) => {
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('server-port-changed', value);
    }
  });
  ...
  ipcMain.on('send-state-to-settings', (event, state) => {
    if (settingsWindow && !settingsWindow.isDestroyed()) {
      settingsWindow.webContents.send('update-server-status', state.isServerConnected);
      settingsWindow.webContents.send('update-recording-status', state.isRecording);
      settingsWindow.webContents.send('update-translation-log', state.translationHistory);
    }
  });
