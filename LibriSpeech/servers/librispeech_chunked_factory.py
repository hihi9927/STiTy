"""
ASR factory for Chunked mode - uses standard Whisper model (downloads if needed)
"""
...
def librispeech_chunked_args(parser):
    group = parser.add_argument_group('Whisper arguments (Chunked mode)')
...
def librispeech_chunked_factory(args):
    ...
    logger.info(f"[CHUNKED MODE] Using model: {a['model_path']}")
