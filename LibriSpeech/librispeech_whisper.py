...
def librispeech_whisper_args(parser):
    group = parser.add_argument_group('Whisper arguments')
    group.add_argument('--model_path', type=str, default=r'c:\Users\VRSTUDIO3\Desktop\STiTy_fk\SimulStreaming\large-v2.pt',
...
class LibriSpeechWhisperASR(ASRBase):
    ...
    def transcribe(self, audio, init_prompt=""):
        ...
        "Instead, use LibriSpeechWhisperOnline.process_iter().")
        raise NotImplementedError("Use LibriSpeechWhisperOnline.process_iter() instead of transcribe().")
...
class LibriSpeechWhisperOnline(OnlineProcessorInterface):
    ...
    def process_iter(self):
        ...
        if generation_progress:
            if 'language' in generation_progress:
