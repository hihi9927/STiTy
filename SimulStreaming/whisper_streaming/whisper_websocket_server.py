...
class WebSocketHandler:
    ...
    async def send_result(self, iteration_output):
        ...
        if iteration_output and iteration_output.get('text'):
            ...
            elif self.detected_language != detected_lang:
                logger.info(f"[Language Change] {self.detected_language} -> {detected_lang}")
                language_changed = True

            logger.info(f"[send_result] Text segment: {text}")
            logger.info(f"[send_result] Detected language: {detected_lang}")

            # If language changed, flush the buffer with old language first
            if language_changed and len(self.translation_buffer) > 0:
                await self.flush_translation_buffer("language_change")
                self.detected_language = detected_lang
